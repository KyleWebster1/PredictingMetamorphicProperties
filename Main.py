...
print("SVM ADD")
scores = cross_validate(svm, addData, al[:len(addData)], cv=10, scoring=('accuracy', 'precision', 'recall', 'f1', 'roc_auc'))
for key in scores:
    print(key, np.average(scores[key])*100, "%")
print("NB ADD")
scores = cross_validate(nb, addData, al[:len(addData)], cv=10, scoring=('accuracy', 'precision', 'recall', 'f1', 'roc_auc'))
for key in scores:
    print(key, np.average(scores[key])*100, "%")
